    ...
    let Select = class Select extends Persisted(Focusable(DataSourced(Input(s$1)))) {
        ...
        render() {
            ...
            <select
                ...
                .value=${this.items.status === i$1.COMPLETE ? this.value || '' : ''}
                @change=${(ev) => (this.value = ev.target.value)}
            >
                ${this.items.render({
