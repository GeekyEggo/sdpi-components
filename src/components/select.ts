...
export class Select extends Persisted(
	Focusable(DataSourced(DynamicValueType(Input<typeof LitElement, boolean | number | string>(LitElement)))),
) {
	/** @inheritdoc */
				...
				${this.items.render({
					...
					complete: () => html`
						...
						${this.renderDataSource(
							...
							(group, children) =>
								html`<optgroup .label=${group.label?.toString() || ""}>${children}</optgroup>`,
						)}
					`,
				})}
			</select>
